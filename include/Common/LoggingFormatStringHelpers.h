...
void formatStringCheckArgsNumImpl(std::string_view str, size_t nargs);
...
struct FormatStringHelperImpl
{
    ...
    template<typename T>
    FormatStringHelperImpl(T && str) : message_format_string(tryGetStaticFormatString(str)), fmt_str(std::forward<T>(str)) /// NOLINT
    {
...
ssize_t formatStringCountArgsNum(const char * const str, size_t len)
{
...
template<typename T>
void formatStringCheckArgsNum(T && str, size_t nargs)
{
